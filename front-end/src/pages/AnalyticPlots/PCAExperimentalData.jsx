...
const PCAExperimentalData = (props) => {
        ...
        if (!loading) {
            ...
            for (const element of features) {
                for (let j = 0; j < element.length; j++) {
                    if (isNaN(element[j]) || element[j] === null || element[j] === undefined) {
                        element[j] = 0;
                    }
                }
            }
    ...
    useEffect(() => {
        if (transformedData) {
            ...
            const transformedDataWithStyle = transformedData.data.map((d, index) => ({
                ...
                eruptive_style: data[index].eruptive_style // Add eruptive_style from data
            }));
            ...
            const groupedData = d3.group(data, d => d.eruptive_style);
    
            groupedData.forEach((particles, eruptiveStyle) => {
                const styleIndices = particles.map(p => data.indexOf(p));
    ...
    }, [transformedData, data, yAxisValue, visibilityMode]);
